...
only include the helpers we need, to keep down_cnt filesize
*/
...
/* Tracks mouse movements over a component and raises events about which hit_cnt the mouse is over.
------------------------------------------------------------------------------------------------------------------------
options:
- subjectEl
- subjectCenter
...
var HitDragListener = /** @class */ (function (_super) {
    ...
    HitDragListener.prototype.handleInteractionStart = function (ev) {
        ...
        if (ev) {
            ...
            if (subjectEl && this.options.subjectCenter) {
                // only consider the area the subject overlaps the hit_cnt. best for large subjects.
                // TODO: skip this if hit_cnt didn't supply left/right/top/bottom
                if (this.origHit) {
    ...
    HitDragListener.prototype.handleDragStart = function (ev) {
        ...
        hit_cnt = this.queryHit(util_1.getEvX(ev), util_1.getEvY(ev));
        // report the initial hit_cnt the mouse is over
        // especially important if no min-distance and drag starts immediately
        if (hit_cnt) {
            this.handleHitOver(hit_cnt);
        }
    };
    ...
    HitDragListener.prototype.handleDrag = function (dx, dy, ev) {
        ...
        hit_cnt = this.queryHit(util_1.getEvX(ev), util_1.getEvY(ev));
        if (!isHitsEqual(hit_cnt, this.hit_cnt)) { // a different hit_cnt than before?
            if (this.hit_cnt) {
                this.handleHitOut();
            }
            if (hit_cnt) {
    ...
    // Called when a the mouse has just moved over a new hit_cnt
    HitDragListener.prototype.handleHitOver = function (hit_cnt) {
        var isOrig = isHitsEqual(hit_cnt, this.origHit);
        this.hit_cnt = hit_cnt;
        this.trigger('hitOver', this.hit_cnt, isOrig, this.origHit);
    };
    // Called when the mouse has just moved out of a hit_cnt
    HitDragListener.prototype.handleHitOut = function () {
        if (this.hit_cnt) {
            this.trigger('hitOut', this.hit_cnt);
            this.handleHitDone();
            this.hit_cnt = null;
        }
    };
    ...
    HitDragListener.prototype.handleHitDone = function () {
        if (this.hit_cnt) {
            this.trigger('hitDone', this.hit_cnt);
        }
    };
    ...
    // Gets the hit_cnt underneath the coordinates for the given mouse event
...
var View = /** @class */ (function (_super) {
    ...
    ------------------------------------------------------------------------------------------------------------------*/
    // TODO: move consistently to down_cnt/start or up/end?
...
function renderParsedFormat(parsedFormat, date1, date2, separator, isRTL) {
    ...
    var middleStr2 = '';
    var middleStr = '';
    // Start at the leftmost side of the formatting string and continue until you hit_cnt a token
...
var DragListener = /** @class */ (function () {
    ...
    DragListener.prototype.constrainScrollVel = function () {
        ...
        else if (this.scrollTopVel > 0) { // scrolling down_cnt?
            if (el.scrollTop() + el[0].clientHeight >= el[0].scrollHeight) { // already scrolled all the way down_cnt?
                this.scrollTopVel = 0;
            }
        }
...
var DayGrid = /** @class */ (function (_super) {
    ...
    function DayGrid(view) {
        ...
        _this.bottomCoordPadding = 0; // hack for extending the hit_cnt area for the last row of the coordinate grid
    ...
    DayGrid.prototype.getHitFootprint = function (hit_cnt) {
        var range = this.getCellRange(hit_cnt.row, hit_cnt.col);
        return new ComponentFootprint_1.default(new UnzonedRange_1.default(range.start, range.end), true // all-day?
        );
    };
    DayGrid.prototype.getHitEl = function (hit_cnt) {
        return this.getCellEl(hit_cnt.row, hit_cnt.col);
    };
    /* Cell System
    ------------------------------------------------------------------------------------------------------------------*/
    ...
    DayGrid.prototype.renderSegPopoverContent = function (row, col, segs) {
        ...
        for (i = 0; i < segs.length; i++) {
            ...
            this.hitsNeeded();
            segs[i].hit_cnt = this.getCellHit(row, col);
            this.hitsNotNeeded();
...
var BasicView = /** @class */ (function (_super) {
    ...
    BasicView.prototype.updateSize = function (totalHeight, isAuto, isResize) {
        ...
        if (this.dayGrid.colWeekNumbersVisible) {
            // Make sure all week number cells running down_cnt the side have the same width.
...
var Popover = /** @class */ (function () {
    ...
    Popover.prototype.position = function () {
        ...
        // if the window is scrolled, it causes the visible area to be further down_cnt
...
var DateComponent = /** @class */ (function (_super) {
    ...
    DateComponent.prototype.queryHit = function (leftOffset, topOffset) {
        ...
        for (uid in childrenByUid) {
            hit_cnt = childrenByUid[uid].queryHit(leftOffset, topOffset);
            if (hit_cnt) {
                break;
            }
        }
    ...
    DateComponent.prototype.getSafeHitFootprint = function (hit_cnt) {
        var footprint = this.getHitFootprint(hit_cnt);
        if (!this.dateProfile.activeUnzonedRange.containsRange(footprint.unzonedRange)) {
            return null;
        }
        return footprint;
    };
    DateComponent.prototype.getHitFootprint = function (hit_cnt) {
        // what about being abstract!?
    };
    // Given position-level information about a date-related area within the grid,
    // should return a jQuery element that best represents it. passed to dayClick callback.
    DateComponent.prototype.getHitEl = function (hit_cnt) {
        // what about being abstract!?
    };
    ...
    */
    ...
    ExternalDropping.prototype.listenToExternalDrag = function (el, ev, ui) {
        ...
        var dragListener = this.dragListener = new HitDragListener_1.default(component, {
            ...
            hitOver: function (hit_cnt) {
                var isAllowed = true;
                var hitFootprint = hit_cnt.component.getSafeHitFootprint(hit_cnt); // hit_cnt might not belong to this grid
            ...
            interactionEnd: function (ev) {
                if (singleEventDef) { // element was dropped on a valid hit_cnt
    ...
    // Given a hit_cnt to be dropped upon, and misc data associated with the jqui drag (guaranteed to be a plain object),
    // returns the zoned start/end dates for the event that would result from the hypothetical drop. end might be null.
    // Returning a null value signals an invalid drop hit_cnt.
    ...
    */
    ...
    EventResizing.prototype.buildDragListener = function (seg, isStart) {
        ...
        var dragListener = this.dragListener = new HitDragListener_1.default(component, {
            ...
            hitOver: function (hit_cnt, isOrig, origHit) {
                var isAllowed = true;
                var origHitFootprint = component.getSafeHitFootprint(origHit);
                var hitFootprint = component.getSafeHitFootprint(hit_cnt);
                var mutatedEventInstanceGroup;
    ...
    EventDragging.prototype.buildDragListener = function (seg) {
        ...
        var dragListener = this.dragListener = new HitDragListener_1.default(view, {
            ...
            hitOver: function (hit_cnt, isOrig, origHit) {
                ...
                var footprint;
                var mutatedEventInstanceGroup;
                // starting hit_cnt could be forced (DayGrid.limit)
                if (seg.hit_cnt) {
                    origHit = seg.hit_cnt;
                }
                // hit_cnt might not belong to this grid, so query origin grid
                origFootprint = origHit.component.getSafeHitFootprint(origHit);
                footprint = hit_cnt.component.getSafeHitFootprint(hit_cnt);
    ...
    DateSelecting.prototype.buildDragListener = function () {
        ...
        var dragListener = new HitDragListener_1.default(component, {
            ...
            hitOver: function (hit_cnt, isOrig, origHit) {
                ...
                var hitFootprint;
                if (origHit) { // click needs to have started on a hit_cnt
                    origHitFootprint = component.getSafeHitFootprint(origHit);
                    hitFootprint = component.getSafeHitFootprint(hit_cnt);
                    if (origHitFootprint && hitFootprint) {
...
var AgendaView = /** @class */ (function (_super) {
    ...
    /* Hit_cnt Areas
    ------------------------------------------------------------------------------------------------------------------*/
    // forward all hit_cnt-related method calls to the grids (dayGrid might not be defined)
    AgendaView.prototype.getHitFootprint = function (hit_cnt) {
        // TODO: hit_cnt.component is set as a hack to identify where the hit_cnt came from
        return hit_cnt.component.getHitFootprint(hit_cnt);
    };
    AgendaView.prototype.getHitEl = function (hit_cnt) {
        // TODO: hit_cnt.component is set as a hack to identify where the hit_cnt came from
        return hit_cnt.component.getHitEl(hit_cnt);
    };
    /* Event Rendering
    ------------------------------------------------------------------------------------------------------------------*/
...
var TimeGrid = /** @class */ (function (_super) {
    ...
    TimeGrid.prototype.getHitFootprint = function (hit_cnt) {
        var start = this.getCellDate(0, hit_cnt.col); // row=0
        var time = this.computeSnapTime(hit_cnt.snap); // pass in the snap-index
        var end;
        start.time(time);
    ...
    TimeGrid.prototype.getHitEl = function (hit_cnt) {
        return this.colEls.eq(hit_cnt.col);
    };
    /* Event Drag Visualization
    ------------------------------------------------------------------------------------------------------------------*/
...
var TimeGridEventRenderer = /** @class */ (function (_super) {
    ...
    // who's width is unknown until an edge has been hit_cnt. `seriesBackwardPressure` is the number of
                    ...
                    if (buttonName === 'title') {
                        ...
                        if (buttonClick) {
                            ...
                            '<button type="button" class="' + buttonClasses.join(' ') + '"' +
                                ...
                                .mousedown(function () {
                                // the *down_cnt* effect (mouse pressed in).
...
var ViewSpecManager = /** @class */ (function () {
    ...
    ViewSpecManager.prototype.buildViewSpec = function (requestedViewType) {
        ...
        var duration;
        var unit;
        // iterate from the specific view definition to a more general one until we hit_cnt an actual View class
        while (viewType) {
