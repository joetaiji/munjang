...
//코멘트 글자수 최대수 제한
function insertContentsMax(spanID, max, formNm,type) {

		

	if(type == 'comment'){
		let totalComment = document.querySelectorAll(".cmt_write.cmt_update")
		totalComment.forEach((target)=>{	
			if(target.classList.contains("update")){
			
				const text = document.querySelectorAll(".cmt_write.cmt_update.update")[0].querySelector('textarea');
				...
				target.querySelector('.textarea_foot span span').innerHTML = text.value.length
			}
		})
	}else{
